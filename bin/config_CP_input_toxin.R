...
meta1 <- tibble::tibble(file = list.files(path = raw_imagesDir),
  ...
  dplyr::filter(is.na(stringr::str_match(file, "(?:[0-9]+-[a-z|A-Z|0-9]+-p[0-9]+-m[0-9]+[X|x]_[A-Z][0-9]+)(_thumb)?(?:|[:word:]{36})\\.([a-z]{3}|[A-Z]{3})")[,2])) %>%
  dplyr::mutate(copy = stringr::str_match(file, "([0-9]+-[a-z|A-Z|0-9]+-p[0-9]+-m[0-9]+[X|x]_[A-Z][0-9]+)(?:|[:word:]{36})")[,2]) %>% # nolint: line_length_linter.
